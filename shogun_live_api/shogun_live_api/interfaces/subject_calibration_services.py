...
class SubjectCalibrationServices(ViconInterface):
    ...
    def capture_session(self, calibration_session_id):
        """Get the capture session associated with this subject calibration session.

        This can be used in conjunction with CaptureServices to get additional information about the capture

        Args:
            calibration_session_id < int >: The calibration session provided by StartSubjectCalibration.

        Return:
            return < Result >: Ok - On success.
                NotFound - The calibration session Id is incorrect.
                NotAvailable - The capture session is not ready yet.
            capture_session_id < int >: A capture session that can be passed to CaptureServices.
        """
        return self.client.send_command("SubjectCalibrationServices.CaptureSession", calibration_session_id)
...
SchemaServices.register_json_schema(SubjectCalibrationServices,"""{"Type": "NamedTuple", "Role": "Function", "TypeName": "SubjectCalibrationServices.CaptureSession", "SubSchemas": [["Return",
                                                                  {"Type": "UInt32", "Role": "Result"}], ["CalibrationSessionId", {"Type": "UInt32", "Role": "Input"}], ["CaptureSessionId",
                                                                  {"Type": "UInt32", "Role": "Output"}]]}""")
